...
const makeMessagesRecvSocket = (config) => {
                            ...
                            if (ws.isOpen) {
                                ...
                                try {
                                    ...
                                    if (err?.message?.includes('Connection Closed') || err?.output?.statusCode === 428) {
                                        logger.debug('connection closed during retry request, ignoring');
                                    } else {
                                        throw err;
                                    }
                                }
    ...
    const fetchMessageHistory = async (count, oldestMsgKey, oldestMsgTimestamp) => {
        ...
        try { return await sendPeerDataOperationMessage(pdoMessage); } catch(err) { if (err?.message?.includes('Connection Closed') || err?.output?.statusCode === 428) { logger.debug('connection closed during history sync PDO, ignoring'); return; } throw err; }
    };
    const requestPlaceholderResend = async (messageKey) => {
        ...
        try { return await sendPeerDataOperationMessage(pdoMessage); } catch(err) { if (err?.message?.includes('Connection Closed') || err?.output?.statusCode === 428) { logger.debug('connection closed during PDO resend, ignoring'); return; } throw err; }
    };
