...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('cards' in message && !!message.cards) {
        ...
        for (const card of message.cards) {
            let cardMedia = {};
            if (card.image) {
                cardMedia = await (0, exports.prepareWAMessageMedia)({ image: card.image }, options);
            } else if (card.video) {
                cardMedia = await (0, exports.prepareWAMessageMedia)({ video: card.video }, options);
            }
            ...
            carouselCards.push(Types_1.WAProto.Message.InteractiveMessage.fromObject(cardInteractive));
        }
        const interactiveMessage = {
            body: { text: message.text || message.body || '' },
